...
int Emulate8080Op(State8080* state)
{
	...
	// printf("%d", instructionNumber++);
	// printf("\n");

  // if (instructionNumber == 539) {
	//   printf("debug\n");
  // }
...
int cycles = 0;
int cyclesUntilInterrupt = 16667;
int lastInterrupt = 1;
...
int main (int argc, char**argv)
{
	...
	ReadFileIntoMemoryAt(state, "C:\\Users\\andys\\Desktop\\8080\\invaders", 0x0);
  ...
  //   state->memory[0x59e] = 0x05;
	...
	while (done != 0)
	{
      ...
      if (cycles >= cyclesUntilInterrupt) {
        if (state->int_enable) {
          if (lastInterrupt == 2) {
        ...
        cycles -= cyclesUntilInterrupt;
      }
    }

  printf("%s", "WE'RE FINISHED HERE");
	return 0;
}